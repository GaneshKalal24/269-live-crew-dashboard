...
export function fmtDMY(iso){ if(!iso)return'—'; const [y,m,d]=iso.split('-'); return d+'/'+m+'/'+y.slice(2); }
...
export const ASBESTOS_ACTS = new Set(['Flooring Encapsulation','Smoke Test','Remove Asbestos',
  'Clearance + Remove Encapsulation','Clearance','Remove Construction Joints - Asbestos']);
...
export function buildLookahead(schedule, fromISO, toISO){
  ...
  for(const [structure, acts] of Object.entries(schedule)){
    for(const a of acts){
      ...
      let status='upcoming';
      if(a.p>=100 || a.f<today) status='done';
      else if(a.s<=today && today<=a.f) status='live';
      else if(a.p>0) status='live';
