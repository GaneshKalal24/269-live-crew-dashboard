...
// Demo look-ahead: all activities from demo_schedule that fall in the window (all are demolition).
export function buildDemoLookahead(demoSchedule, fromISO, toISO){
  const today = todayISO();
  const out = [];
  for(const [structure, acts] of Object.entries(demoSchedule)){
    acts.forEach((a, idx)=>{
      if(!a.s || !a.f) return;
      if(a.f < fromISO || a.s > toISO) return;
      let status='upcoming';
      if(a.p>=100 || a.f<today) status='completed';
      else if(a.s<=today && a.p>0) status='live';
      out.push({structure, activity:a.a, s:a.s, f:a.f, p:a.p||0, status, idx});
    });
  }
  out.sort((x,y)=> x.s.localeCompare(y.s) || x.structure.localeCompare(y.structure));
  return out;
}
